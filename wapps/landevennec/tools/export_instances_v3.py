...
Transform du point, indépendante de la position de l'objet source :
  Le nœud "Object Info" référence un objet (ex: "Chapiteau Quadruple Exp")
  qui peut lui-même se trouver n'importe où dans la scène. On veut la
  transform du *point* d'instanciation, indépendamment de cette position
  de référence, pour qu'elle s'applique correctement au .glb exporté
  séparément (exporté à l'origine). Pour chaque instance, on calcule donc :
      M_point = M_monde_instance @ M_monde_objet_source⁻¹
  ce qui retire la transform propre de l'objet référencé par "Object Info".
...
def export_instances():
    ...
    for inst in depsgraph.object_instances:
        ...
        # Transform du point = transform monde de l'instance moins la
        # transform propre de l'objet référencé par "Object Info".
        m_source = inst.object.matrix_world
        m_point = inst.matrix_world @ m_source.inverted()
        m_point = _convert_matrix(m_point)
